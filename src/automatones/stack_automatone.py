...
def generate_stack_automaton(
    ...
    for i in range(number_of_assign):
        ...
        for j in range(number_of_assign):
            q_i_j : State = State(f"q_{i}_{j}")
            states.add(q_i_j)
            transitions[q_i_j] = dict()
            for letter in full_alphabet:
                transitions[q_i_j][letter] = q_i_j
            transitions[q_i][f"assign_{j + 1}_open"] = q_i_j
            transitions[q_i][f"assign_r_{j + 1}_open"] = q_i_j
            for k in range(number_of_assign):
                transitions[q_i_j][f"assign_{k + 1}_open"] = qf
                transitions[q_i_j][f"assign_r_{k + 1}_open"] = qf
                if k == j:
                    transitions[q_i_j][f"assign_{j + 1}_close"] = q_i
                    transitions[q_i_j][f"assign_r_{j + 1}_close"] = q_i
                else:
                    transitions[q_i_j][f"assign_{k + 1}_close"] = qd
                    transitions[q_i_j][f"assign_r_{k + 1}_close"] = qd
    return Automaton(
